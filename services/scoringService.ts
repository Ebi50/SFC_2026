...
const calculateTimeTrialPoints = (
  ...
  // Step 2: Identify valid finishers, calculate their time (with or without handicap), and sort them.
  ...
  const rankedFinishers = eventResults
    ...
    .map(r => {
      ...
      if (event.eventType === EventType.BZF) {
        // BZF: Use absolute time only (no handicaps)
        adjustedTimeSeconds = r.timeSeconds || 0;
      } else {
        // EZF: Use handicap-adjusted time
        const handicap = calculateHandicap(participant, r, event, settings);
        adjustedTimeSeconds = (r.timeSeconds || 0) + handicap;
      }
...
export const calculateOverallStandings = (
    ...
    // Determine if the season is closed (all events should have a season)
    const seasonNumbers = Array.from(new Set(events.filter(e => e.finished).map(e => e.season)));
    const currentSeason = seasonNumbers.length > 0 ? seasonNumbers[0] : new Date().getFullYear();
    const isSeasonClosed = settings.closedSeasons?.includes(currentSeason) ?? false;

    Object.values(standingsByParticipant).forEach(standing => {
        // Drop Score Logic (NEW):
        // 1. Participants with only 1 event participation get NO drop score
        // 2. Drop scores are only applied when the season is closed
        // 3. A missed event automatically counts as a drop score
        // 4. We drop the worst scores from attended events if fewer events were missed than allowed drop scores
        ...
        // NEW: Only apply drop scores if season is closed AND participant has more than 1 event
        if (isSeasonClosed && attendedEventCount > 1) {
        ...
        standing.tieBreakerScores = standing.results
            .filter(r => !r.isDropped)
            .map(r => r.points)
            .sort((a, b) => b - a);
    });
