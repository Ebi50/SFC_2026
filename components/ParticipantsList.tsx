          ...
          <table className="w-full text-left">
            <thead className="gradient-dark">
              <tr>
                ...
                {isAdmin && <th className="p-4 font-display font-semibold text-sm text-white uppercase tracking-wider text-center">Foto</th>}
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredParticipants.map((p) => (
                <tr key={p.id} className="hover:bg-primary/10">
                  ...
                  {isAdmin && <td className="p-4 text-center">
                    {(p as any).fotoConsent ? <CheckIcon className="w-6 h-6 text-green-600 mx-auto" /> : <span className="text-gray-400">✗</span>}
                  </td>}
              ...
              {participants.length === 0 && (
                <tr>
                  <td colSpan={isAdmin ? 10 : 6} className="p-4 text-center text-gray-500">
