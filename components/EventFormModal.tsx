...
export const EventFormModal: React.FC<EventFormModalProps> = ({
    ...
    const handleSaveClick = () => {
        ...
        // Include season: use existing event's season or selectedSeason for new events
        const eventToSave = {
            ...formData,
            id: eventId,
            season: event?.season || selectedSeason
        };
        
        onSave(eventToSave, results, teams, teamMembers);
    };
    ...
    const renderResultsEditor = () => {
        ...
        const winnerRanks: (1 | 2 | 3)[] = [1, 2, 3];
        const isBZF = formData.eventType === EventType.BZF;
    
        // Sort results by time (absolute for BZF, adjusted for EZF)
        const sortedResults = [...results].sort((a, b) => {
            ...
            if (!participantA || !participantB || !a.timeSeconds || !b.timeSeconds) {
                // If no time data, sort by participant name
                return getParticipantName(a.participantId).localeCompare(getParticipantName(b.participantId));
            }
            
            if (isBZF) {
                // BZF: Sort by absolute time only
                return a.timeSeconds - b.timeSeconds;
            } else {
                // EZF: Sort by handicap-adjusted time
                const fakeEvent: Event = { ...formData, id: event?.id || '', season: event?.season || selectedSeason };
                const handicapA = calculateHandicap(participantA, a, fakeEvent, settings);
                const handicapB = calculateHandicap(participantB, b, fakeEvent, settings);
                const adjustedTimeA = a.timeSeconds + handicapA;
                const adjustedTimeB = b.timeSeconds + handicapB;
            ...
            <div>
                ...
                {isBZF ? (
                    <div className="grid grid-cols-12 gap-2 items-center mb-2 px-2 text-sm font-semibold text-gray-600">
                        <div className="col-span-4">Teilnehmer</div>
                        <div className="col-span-3">Zeit (mm:ss)</div>
                        <div className="col-span-3">Bonus-Rang</div>
                ...
                {sortedResults.map(result => {
                    ...
                    if (participant && result.timeSeconds && !isBZF) {
                    ...
                    return isBZF ? (
                        <div key={result.id} className="grid grid-cols-12 gap-2 items-center mb-2 p-2 bg-gray-50 rounded">
                            <div className="col-span-4 font-medium">{getParticipantName(result.participantId)}</div>
