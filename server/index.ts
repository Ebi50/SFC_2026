...
import { initializeDatabase, shutdownDatabaseSync } from './dbSync';
...
async function startServer() {
  try {
    // Initialize database sync (download from Cloud Storage in production)
    if (process.env.NODE_ENV === 'production' && process.env.GCS_BUCKET_NAME) {
      console.log('☁️  Production mode: Syncing database with Cloud Storage...');
